...
!!! warning "`resource` and `http_verb` are v3-only"
...
!!! danger "v3 does not work yet for agents registered with `endpoints`"
...
!!! warning "Breaking: `X402TokenOptions` is no longer an `MppTokenOptions`"
...
!!! warning "Breaking: `X402TokenOptions` now rejects unknown keyword arguments"
...
6. **Branch on `billing_model`**: Never decide "was the buyer charged?" from `credits_redeemed`
