...
`description` / `mime_type`). Sending it is worthwhile on v2 too: without
`resource.url` the backend skips endpoint validation for the token.
...
MPP works the other way round: `token_version` behaves identically on
`POST /api/v1/mpp/permissions`, but **do not request v3 there**. One MPP access
token is reused across many challenges — distinct operations — so a one-time
nonce consumed by the first settle would kill the buyer's second challenge.
`payments.mpp.fetch` never asks for v3, and mints a fresh token per challenge
turn anyway.
...
- [OAuth 401 vs. payment-required](09-mcp-integration.md#oauth-401-vs-payment-required) - In-band x402 v2 MCP signaling
