...
def test_get_asset_ddo(payment):
    response = payment.get_asset_ddo(
        did="did:nv:a0079b517e580d430916924f1940b764e17c31e368c509483426f8c2ac2e7116"
    )
    assert response.status_code == 200


def test_get_service_token(payment):
    response = payment.get_service_token(
        service_did="did:nv:349b6ec01dc8cfdc160d2b71bbfb7e6e93963206e7ab682128733360c0d92ac6"
    )
    assert isinstance(response, ServiceTokenResultDto)


def test_order_plan(payment):
    response = payment.order_plan(
        plan_did="did:nv:b508b82ccb0572feb52ed0f0af11b3c56358500e937b4900e36500e8e3588315"
    )
    assert isinstance(response, OrderPlanResultDto)


def test_download_file(payment):
    response = payment.download_file(
        file_did="did:nv:64e669c1a433ec68e242494a9471176c977c38bef2e8b09c109530263b0df65f",
        destination="/tmp",
    )
...
def test_get_plan_balance(payment):
    response = payment.get_plan_balance(
        plan_did="did:nv:a0079b517e580d430916924f1940b764e17c31e368c509483426f8c2ac2e7116",
        account_address="0x4fe3e7d42fA83be4E8cF03451Ac3F25980a73fF6",
    )
    assert isinstance(response, BalanceResultDto)


def test_mint_credits(payment):
    response = payment.mint_credits(
        plan_did="did:nv:e405a91e3152be1430c5d0607ebdf9236c19f34bfba0320798d81ba5f5e3e3a5",
        amount="12",
        receiver="0x4fe3e7d42fA83be4E8cF03451Ac3F25980a73fF6",
    )
    assert isinstance(response, MintResultDto)


def test_burn_credits(payment):
    response = payment.burn_credits(
        plan_did="did:nv:e405a91e3152be1430c5d0607ebdf9236c19f34bfba0320798d81ba5f5e3e3a5",
        amount="12",
    )
