...
class CreateTaskDto(BaseModel):
    ...
    input_additional: Optional[Dict[str, Any]] = Field(None, example={"assistantId": "1234"}, description="Additional key-value parameters required for the task")
    input_artifacts: Optional[List[Artifact]] = Field(None, example=[{"artifactId": "art-aabb", "url": "https://nevermined.io/file.txt"}], description="Artifacts for the task")
...
class BaseStepDto(BaseModel):
    ...
    input_additional: Optional[Dict[str, Any]] = Field(None, example={"assistantId": "1234"}, description="Additional key-value parameters required for the step")
    ...
    predecessor: Optional[str] = Field(None, description="Previous step id. If not given, the system will associate it with the latest step (by order).")
    is_last: Optional[bool] = Field(None, example=True, description="Is the last step of the task?")


class NewStepDto(BaseStepDto):
    step_id: Optional[str] = Field(None, description="Id of the step. If not given or invalid, the system will auto-generate it")
    step_status: Optional[AgentExecutionStatus] = Field(None, example=AgentExecutionStatus.Not_Ready, description="Status of the step")


class UpdateStepDto(BaseStepDto):
    step_id: str = Field(..., description="Id of the step. If not given or invalid, the system will auto-generate it")
    did: Optional[str] = Field(None, example="did:nv:1234", description="The id of the agent (DID) to which the step belongs")
    step_status: AgentExecutionStatus = Field(..., example=AgentExecutionStatus.Completed, description="New status of the step")
    output: Optional[str] = Field(None, example="success", description="Output of the step")
    output_additional: Optional[Dict[str, Any]] = Field(None, example={"message": "success"}, description="Additional key-value output values generated by the step")
...
class CreateStepsDto(BaseModel):
    steps: Optional[List[NewStepDto]] = Field(None, description="List of new Steps to create")    
...
class TaskEntityDto(BaseModel):
    ...
    input_additional: Any = Field(default_factory=dict, description="Additional input data")
    ...
    output_additional: Dict[str, Any] = Field(default_factory=dict, description="Additional output data")
    output_artifacts: List[Any] = Field(default_factory=list, description="List of output artifacts")
